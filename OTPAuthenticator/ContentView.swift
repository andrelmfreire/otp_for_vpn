...
// Your actual OPNsense configuration
let otpAuthURL = "otpauth://totp/andre.freire@OPNsense?secret=RGKFLVKQL4JLZROYKWVV4EWTKQJBLCXO"
let secret = extractSecret(from: otpAuthURL) ?? "RGKFLVKQL4JLZROYKWVV4EWTKQJBLCXO"
...
struct ContentView: View {
    ...
    func copyToClipboard() {
        let baseString = "Wequ3kahtee@ph9o"
        let finalPassword = "\(baseString)\(otpCode)"
    ...
    var body: some View {
        VStack(spacing: 24) {
            Text("OPNsense Authenticator")
                .font(.title)
                .fontWeight(.bold)
                .padding(.top, 20)
            
            Text("andre.freire@OPNsense")
                .font(.subheadline)
